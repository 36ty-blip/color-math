...
Obsidian Color Math is a local Python CLI that adds scoped `\textcolor{...}{...}` wrappers to display-math blocks while preserving the original LaTeX structure.
...
*Rendered MathJax example using LaTeX generated by the CLI; this is not an Obsidian screenshot.*
...
Requires Python 3.10 or newer and Obsidian with its built-in MathJax rendering.
...
If you already use `pipx`, `pipx install .` installs the CLI in an isolated environment.

Preview a converted note without changing it:

```bash
color-math --file "path/to/note.md"
```

Write the conversion back only after reviewing the preview:
...
Edit the `COLORS` dictionary in [`color_math/config.py`](https://github.com/36ty-blip/obsidian-color-math/blob/main/color_math/config.py):
...
```bash
color-math --file --undo --in-place "path/to/colored-note.md"
```
...
- `color-math --self-test` runs the installed-package smoke tests.
...
Both commands are read-only by default. Refresh [`tests/generated/`](https://github.com/36ty-blip/obsidian-color-math/blob/main/tests/README.md) explicitly:

```bash
python -m tests.self_test --update-generated
```

GitHub Actions runs the suite on Windows and Linux. Build the wheel and source distribution with the standard frontend:
...
├── docs/
│   ├── assets/
│   │   └── obsidian-color-math-preview.png
│   ├── examples.md                 # Additional usage examples
│   └── release-notes-v0.1.1.md     # Draft notes for the next release
├── .github/
│   └── workflows/
...
See [`tests/README.md`](https://github.com/36ty-blip/obsidian-color-math/blob/main/tests/README.md) for the fixture layout and [`COMMUNITY_POST.md`](https://github.com/36ty-blip/obsidian-color-math/blob/main/COMMUNITY_POST.md) for the community announcement draft.
